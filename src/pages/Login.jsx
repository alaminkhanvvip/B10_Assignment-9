...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      toast.error(`${TOAST_MESSAGES.LOGIN_ERROR}: ${error.message}`);
    }
    setLoading(false);
  };
  ...
  return (
    <>
      <PageTitle title="Login" description="Access your dashboard and start making a difference in vulnerable communities." />
      <div className="hero min-h-screen bg-base-200">
      <div className="hero-content flex-col lg:flex-row-reverse">
        <div className="text-center lg:text-left">
          <h1 className="text-5xl font-bold text-primary">❄️ Welcome Back!</h1>
          <p className="py-6 text-lg">Access your dashboard and continue making a difference in vulnerable communities across Bangladesh.</p>
        ...
        <div className="card shrink-0 w-full max-w-sm shadow-2xl bg-base-100">
          <form className="card-body" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="form-control">
              ...
              <label className="label">
                <Link to="/forgot-password" state={{ email: getValues('email') }} className="label-text-alt link link-hover">
                  Forgot password?
                </Link>
              </label>
            </div>
